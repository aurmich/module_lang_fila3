...
Inoltre, data la limitazione del fallback con i file JSON, raccomando di continuare a utilizzare principalmente file PHP per le traduzioni strutturate, come discusso nella documentazione precedente (`/var/www/html/_bases/base_techplanner_fila3_mono/laravel/Modules/Lang/project_docs/static-text-translation.md`). I file JSON possono essere utilizzati per testi più lunghi, ma con la consapevolezza che il fallback non funzionerà come previsto.
