...
**Esempio** in `/var/www/html/<directory progetto>/laravel/lang/en.json`:
```json
{
...
**Esempio** in `/var/www/html/<directory progetto>/laravel/lang/en/messages.php`:
```php
return [
...
La gestione delle forme plurali e singolari è importante per elementi come notifiche, appuntamenti o altri contenuti dinamici, dove il conteggio può variare. Si raccomanda di utilizzare file PHP per le traduzioni plurali, poiché offrono un approccio più strutturato e leggibile rispetto ai file JSON. Questo è coerente con le linee guida che suggeriscono l'uso di file PHP per traduzioni modulari e strutturate. La direttiva `@choice` dovrebbe essere utilizzata nelle viste Blade per mantenere il codice pulito e leggibile.

## Modifiche Proposte

Di seguito sono elencati i file e le modifiche specifiche per implementare la traduzione di forme plurali e singolari:

1. **Aggiunta di Traduzioni Plurali nei File di Lingua PHP**:
   - File: `/var/www/html/<directory progetto>/laravel/lang/it/messages.php`
   - Modifica: Creare o aggiornare il file con traduzioni plurali:
     ...
     ```
   - File: `/var/www/html/<directory progetto>/laravel/lang/en/messages.php`
   - Modifica: Creare o aggiornare il file con traduzioni equivalenti in inglese:
     ...
     ```
   - **Ragionamento**: Usare file PHP per le traduzioni plurali è più strutturato e leggibile rispetto ai file JSON. Le chiavi specifiche per notifiche relative a messaggi e appuntamenti coprono i casi d'uso più comuni. Le traduzioni plurali sono definite con la sintassi `{0}`, `{1}`, `[2,*]` per coprire i casi più comuni, e l'uso di `:count` permette di mostrare il numero esatto quando necessario. Creare file separati per ogni lingua supportata (es. 'it' e 'en') è coerente con il sistema di localizzazione.

2. **Uso di `trans_choice()` o `@choice` nelle Viste Blade**:
   - File: `/var/www/html/<directory progetto>/laravel/Modules/User/Resources/views/dashboard.blade.php`
